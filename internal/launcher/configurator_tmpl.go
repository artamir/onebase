...
<div class="tabs">
  ...
  <a class="tab {{if eq .Tab "convert"}}active{{end}}" href="/bases/{{.Base.ID}}/configurator?tab=convert">🔄 {{t $.Lang "Конвертер 1С"}}</a>
...
<div class="cfg-right">
  ...
  <div class="cfg-panel" id="e-{{.Name}}">
    ...
    {{template "entity-detail" (dict "Entity" . "BaseID" $.Base.ID "ConfigSource" $.Base.ConfigSource "ModuleSaved" $.ModuleSaved "ModuleSavedEntity" $.ModuleSavedEntity "AllEntityNames" $.AllEntityNames "AllEnumNames" $.AllEnumNames "FieldsSaved" $.FieldsSaved "FieldsSavedEntity" $.FieldsSavedEntity "Lang" $.Lang)}}
  </div>
  {{end}}
...
<div class="convert-form">
  <h3>🔄 {{t $.Lang "Конвертация конфигурации 1С → onebase"}}</h3>
  <form method="POST" action="/bases/{{.Base.ID}}/configurator/convert">
    <div class="fg">
      <label>{{t $.Lang "Путь к папке выгрузки 1С"}}</label>
      <input type="text" name="src_dir" value="{{.ConvertSrcDir}}"
             placeholder="C:\Users\...\1C\МояКонфигурация" autofocus>
      <div class="hint">{{t $.Lang "В 1С: Конфигуратор → Конфигурация → Выгрузить конфигурацию в файлы"}}</div>
    </div>
    <div class="form-btns">
