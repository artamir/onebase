...
func TestMigrateLegacyPrintForms_ConvertsAndDeletes(t *testing.T) {
	...
	converted, err := migrateLegacyPrintForms(dir, false)
	if err != nil {
	...
	if err != nil {
		t.Fatalf("новый .layout.yaml не создан: %v", err)
	}
	s := string(data)
...
func TestMigrateLegacyPrintForms_KeepFlag(t *testing.T) {
	...
	if _, err := migrateLegacyPrintForms(dir, true); err != nil {
		t.Fatalf("migrate --keep: %v", err)
	}
...
func TestMigrateLegacyPrintForms_NoDir(t *testing.T) {
	...
	converted, err := migrateLegacyPrintForms(dir, false)
	if err != nil {
		t.Fatalf("migrate без printforms: %v", err)
	}
	if len(converted) != 0 {
